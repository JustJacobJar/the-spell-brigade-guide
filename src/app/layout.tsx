      ...
      >
        <div className="flex flex-row">
          ...
          <div className="flex-grow flex-col">
            ...
            <div className="mt-24 ml-64 flex h-full grow">{children}</div>
          </div>
        </div>
        {/* <SpeedInsights /> */}
        {/* <Analytics /> */}
...
function SideBar() {
  const links = [
    { text: "Home", href: "/" },
    { text: "Meta Report", href: "/meta-report" },
    { text: "Guides", href: "/guide-list" },
    { text: "Tierlist Maker", href: "/tierlist" },
    { text: "Create Guide", href: "/curator/guide/create" },
  ];

  return (
    <div className="fixed flex h-full w-64 flex-col border-r-2 border-neutral-800 bg-neutral-900 px-2">
      <div className="h-24 w-full shrink-0 place-content-center text-center text-2xl font-bold">
        Moon`s Grimoire
      </div>
      <div className="flex h-full flex-col place-content-between">
        <div className="flex flex-col gap-2 divide-y-2 divide-dashed border-y-2">
          {links.map((link, index) => {
            return <SideLink key={index} href={link.href} text={link.text} />;
          })}
        </div>
...
function TopBar() {
  return (
    <div className="fixed z-50 ml-64 flex h-24 w-screen border-b-2 border-neutral-800 bg-neutral-900">
      <></>
    </div>
  );
}

function SideLink({ text, href }: { text: string; href: string }) {
  return (
    <Link
      className="flex h-12 w-full place-content-center place-items-center"
      href={href}
    >
      {text}
    </Link>
